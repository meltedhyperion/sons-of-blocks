...
export const getAllUserDataUsingMetamaskId = async (
  ...
  export const updateUserDataByMetamaskId = async (
    ...
    if (data.DriversLicenseNumber) update['DriversLicenseNumber'] = data.DriversLicenseNumber;
  ...
  export const isMetamaskIdNew = async (
    metamaskId: string
  ) : Promise<Boolean> => {
  ...
  export const verifyUserData = async (
    ...
    const db = await getDatabase();
    await db.collection('data').updateOne({ MetamaskID: metamaskId }, { $set: {Verified:true} }, { upsert: false });
    const data = await db.collection("data").findOne({ MetamaskId: metamaskId });
    return data;
  };
